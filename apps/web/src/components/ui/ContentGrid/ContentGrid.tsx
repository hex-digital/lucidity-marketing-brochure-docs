...
export function ContentGrid({ content }: { content: GridContent[] }) {
  return (
    <div className="relative w-full">
      <div className="pointer-events-none absolute inset-0 z-content-grid-background grid grid-cols-1 md:grid-cols-3">
        {content.map((item) => (
          <div key={`bg-${item.key}`} />
        ))}
      </div>

      <div className="relative z-content-grid-foreground grid grid-cols-1 md:grid-cols-3">
        {content.map((item) => (
          <div
            key={item.key}
